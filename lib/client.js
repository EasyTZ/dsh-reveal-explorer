window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// 外部 store 的方法可能依赖 `this`（dsh 0.1.2 起的 workspace store 就是
		// 如此）。React 会把回调当裸函数调用，统一包一层以保留接收者。
		...
		const tagId = "dsh-reveal-explorer/button.css";
		if (typeof document !== "undefined" && document.querySelector("style[data-plugin-css=" + JSON.stringify(tagId) + "]") === null) {
			const tag = document.createElement("style");
			tag.dataset.plugin = "dsh-reveal-explorer";
		...
		function RevealButton({ sessionId, workspacesList, t }) {
			...
			const onClick = react.useCallback(async () => {
				...
				setState("opening");
				try {
					const result = await postJson("/api/dsdesktop/reveal-explorer", { workspaceId: owner.workspaceId });
					if (result.ok) {
						setState("idle");
					} else {
						setState("error");
